...
import CateringTable from 'src/components/tables/CateringTable';
...
function CateringForm() {
  ...
  const [myCatering, setMyCatering] = useState('');
  ...
  const getTableData = async () => {
    await axios.get(`https://x8ki-letl-twmt.n7.xano.io/api:_G_SfNPu/catering`,{
      params:{
        users_id: userid
      }
    }).then((response) => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    axios.post(`https://x8ki-letl-twmt.n7.xano.io/api:_G_SfNPu/catering`, defaultValues).then((response) => {
      if (response.status === 200) {
  ...
  return (
    <div>
      <Page title="Input Wedding Organizer">
        ...
        <Card>
          <CardContent>
            <CateringTable TableHead={TablePageHeading} TableContent={tableData} />
          </CardContent>
        </Card>
      </Page>
    </div>
